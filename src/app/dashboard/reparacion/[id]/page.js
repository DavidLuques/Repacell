  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center gap-4 border-b border-violet-500/20 pb-4">
        <Link href="/dashboard" className="p-2 text-violet-400 hover:text-white hover:bg-violet-900/40 rounded-full transition-colors">
          <ArrowLeft size={24} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-white drop-shadow-sm">Taller: Reparación #{(appointment.id).split('-')[0]}</h1>
          <p className="text-violet-300/60 text-sm">Ingresado el {appointment.scheduled_date} a las {appointment.scheduled_time}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-black/40 backdrop-blur-md p-6 rounded-xl shadow-lg border border-violet-500/20">
            <h3 className="text-sm font-semibold text-violet-400 uppercase tracking-wider mb-4 border-b border-violet-500/20 pb-2 flex items-center gap-2">
          ...
          <div className="bg-black/40 backdrop-blur-md p-6 rounded-xl shadow-lg border border-violet-500/20">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="font-medium text-white">{appointment.device_brand} {appointment.device_model}</p>
              </div>
              <div className="pt-2">
                <h4 className="text-xs text-red-400 flex items-center gap-1 font-bold mb-1">
                  <AlertTriangle size={14} /> Falla Reportada (Ingreso)
                </h4>
                <p className="text-sm text-red-200 bg-red-950/30 p-3 rounded-lg border border-red-500/30 italic backdrop-blur-sm">
